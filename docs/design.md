...
| Decision | Choice | Why |
| --- | --- | --- |
| Env vars | `OPENCLAW_WA_SDK_BASE_URL` + `OPENCLAW_WA_SDK_TOKEN` only | Match the real gateway exactly; `accountId` is never used in practice (server defaults to `"default"`). |
